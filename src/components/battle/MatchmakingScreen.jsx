...
import { Swords, X, Radio, Shield, Users } from "lucide-react";

export default function MatchmakingScreen({ onCancel }) {
  const { user } = useAuth();
  const { queueStatus, onlineCount } = useBattleSocket();
    ...
    <div className="max-w-xl mx-auto py-12 px-4 text-center animate-in fade-in duration-300">
      <div className="rounded-3xl border border-zinc-800 bg-[#080c09] p-8 md:p-12 shadow-2xl relative overflow-hidden">
        ...
        <p className="text-sm text-zinc-400 mt-2">
          Buscando un rival de tu nivel en la red competitiva de Jackal...
        </p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 my-8 text-left">
          ...
          <div className="p-3.5 rounded-xl bg-zinc-950 border border-zinc-800 col-span-2 sm:col-span-1">
            ...
            <span className="text-sm font-black text-zinc-200 mt-1 block truncate">General Cyber</span>
            <span className="text-[10px] text-zinc-400">5 Rounds 1v1</span>
          </div>
        </div>
        ...
        <button
          onClick={onCancel}
          className="w-full sm:w-auto min-w-[200px] h-12 px-6 rounded-xl border border-zinc-700 bg-zinc-900/80 hover:bg-zinc-800 text-zinc-200 hover:text-white text-sm font-bold inline-flex items-center justify-center gap-2 transition-colors cursor-pointer"
        >
        ...
        <div className="mt-6 flex items-center justify-center gap-2 text-xs text-zinc-400">
          <Users size={14} />
          <span>{onlineCount} analistas en línea en el ecosistema</span>
        </div>
      </div>
    </div>
  );
